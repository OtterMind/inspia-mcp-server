...
生产修复更新：2026-09-07 已部署源版本 `8c39eeb`。公网 5 工具冒烟与 7 项边界检查通过；360 字中文查询连续三页返回 9 个不同结果，旧版 v1 游标也通过跨部署验证。检索质量仍由后续接口改进处理。以下保留原始问题记录和本地修复过程，不代表分页缺陷仍存在于当前生产。
...
搜索质量问题按后续接口修复处理，本次未改动。该本地复验时生产仍运行 `2c01ea2`，随后已完成开头记录的生产更新。
